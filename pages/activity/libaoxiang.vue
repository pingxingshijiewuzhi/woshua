<template>
	<view class="box">
		<view class="border_bgc">
			<image class="image_top" :src="imgUrl+productobj.detail_images" mode=""></image>
			<view class="border_left">
				...
				<view class=" border_bootom flexs1">
					...
					<view class="border_num">{{'/'+productobj.norms}}</view>
				</view>
			</view>
		</view>
		<view class="barner">
	...
	export default {
		...
		methods: {
			_getproductdetail() {
				...
				this.$common.requests('/api/giftproduct/get_product_detail', obj).then(res => {
					if (res.data.code == 1) {
						// console.log(this.id)
			...
			tobuy(id) {
				uni.navigateTo({
					url: "../activity/jiaodingdan?id=" + id
				})
			},
		},
	}

<template>
	<view class='chukudetail'>
		...
		<view class='chukudetail_header'>
			<view class='imgbox'>
				<image src="../../static/pic/icon_d_cg@2x.png" mode=""></image>
			</view>
			<view class='opacitytitle'>出库成功</view>
		</view>
		<view class='chukudetail_content'>
		...
		<block v-if='mylengths != 0'>
			<view class='chukudetail_content' @click='screenmore'>
				<view class='contetn_item'>
					<view class='item_left'>新机编号</view>
					<view class='arrowsbox'>
						<view>{{detailobj.pos[0]}}</view>
		...
		<view  v-if='showmoreStatus' class='morebox'>
			...
			<scroll-view class='content_time' scroll-y="true">
				<block v-for='(item,index) in detailobj.pos' :key='index'>
	...
	export default {
		...
		methods: {
			_getdetail() {
				...
				this.$common.requests('/api/pos/get_transfer_detail', obj).then(res => {
					if (res.data.code == 1) {
						console.log(res.data.data)
						this.detailobj = res.data.data
						this.mylengths = res.data.data.pos.length
					} else {

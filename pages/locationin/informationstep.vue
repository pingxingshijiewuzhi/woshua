	...
	<view class="box">
		...
		<view class="next_border">
			...
			<view class="next_list flexs1">
				...
				<view class="next_bottom flexs2">
					<input type="number" v-model="merchantphone" placeholder="155****1778" />
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			getData() {
				...
				this.$common.requests('/api/shops/get_step1_info', obj).then(res => {
					if (res.data.code == 1) {
						this.merchantmingcheng = res.data.data.shops_introduction;
						this.merchantname = res.data.data.contacts_name;
						this.merchantphone = res.data.data.contacts_phone;
					} else {
			...
			todairu() {
				if (/^1[345789]\d{9}$/.test(this.merchantphone)) {
					let obj = {
					...
					this.$common.requests('/api/shops/teep_second_update', obj).then(res => {
						if (res.data.code == 1) {
							uni.navigateTo({

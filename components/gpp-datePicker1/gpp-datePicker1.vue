<template>
	<view class="gpp-datePicker" @click.stop="show">
		<slot />
		<view class="g-dp-mask"  v-if='pipkerShowFlag' @click.stop="hide" @touchmove.stop.prevent catchtouchmove="true"></view>
		<view class="g-dp-content"  v-if='pipkerShowFlag' @touchmove.stop.prevent catchtouchmove="true">
	...
	export default {
		...
		methods: {
			...
			wrapperChange(e){
				...
				let selectedDate = this.years[detailValue[0]]+"-"+this.months[detailValue[1]];
				if(this.selectedValue[0] != detailValue[0]){
					this.getMonths(selectedDate);
					selectedDate = this.years[detailValue[0]]+"-"+this.months[detailValue[1]];
